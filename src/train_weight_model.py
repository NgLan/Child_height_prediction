# --- START OF FILE src/train_weight_model.py ---
import pandas as pd
import pickle
from darts import TimeSeries
from darts.models import LinearRegressionModel

def train_linear_regression_model_for_weight(df, model_save_path):
    """Huấn luyện mô hình Linear Regression để dự đoán cân nặng."""
    # Gọi hàm train chung, chỉ thay đổi target và danh sách covariates mặc định
    target_col = 'wtkg'
    # Covariates cho cân nặng: htcm, haz, waz, sex, gagebrth
    covariate_cols = ['htcm', 'haz', 'waz', 'sex', 'gagebrth']
    
    # Về cơ bản, logic giống hệt train_linear_regression_model, chỉ khác target và covariates
    # Thay vì copy-paste, ta có thể gọi hàm chung đó.
    # Nhưng để giữ file riêng biệt theo yêu cầu, tôi sẽ copy và sửa.
    ...
    for subjid, group_original in grouped:
        ...
        try:
            ...
            series = TimeSeries.from_dataframe(
                group, value_cols=[target_col], freq='30D'
            )
            ...
            if actual_covariate_cols:
                 covariates = TimeSeries.from_dataframe(
                    group, value_cols=actual_covariate_cols, freq='30D'
                )
            
            if len(series) < min_len_for_series: continue
    ...
    for subjid_key, current_series in series_dict.items():
        ...
        if len(current_series) < 3: continue
        split_point = int(len(current_series) * 0.8)
        if split_point < 2 or (len(current_series) - split_point < 1): continue
...
# --- END OF FILE src/train_weight_model.py ---